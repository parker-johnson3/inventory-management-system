...
class TestAirplane(unittest.TestCase):
    ...
    def test_airplane_post(self):
        with requests.post('http://127.0.0.1:15000/airplane') as resp:
            ...
            expected_msg = r'success\s+on\s+"/airplane"\s+with\s+method\s+post'
    ...
    def test_airplane_put_with_id(self):
        ...
        with requests.put(f'http://127.0.0.1:15000/airplane/{x}') as resp:
            ...
            expected_msg = r'success\s+on\s+"/airplane/[{]id[}]"\s+with\s+method\s+put'
            self.assertRegex(text, expected_msg)
    ...
    def test_airplane_delete_with_id(self):
        ...
        with requests.delete(f'http://127.0.0.1:15000/airplane/{_id}') as resp:
            self.assertTrue(resp.ok)
            self.assertEqual(resp.status_code, 200)
            self.assertDictEqual({'success': True}, resp.json())

        with requests.get(f'http://127.0.0.1:15000/airplane/{_id}') as resp:
            self.assertFalse(resp.ok)
